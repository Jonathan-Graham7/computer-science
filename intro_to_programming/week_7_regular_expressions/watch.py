...
For instance, if you visit https://youtu.be/xvFZjo5PgG0 on a laptop or desktop, click Share, and then click Embed,
you'll see HTML (the language in which web pages are written) like the below, which you could then copy into your own website's source code,
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/xvFZjo5PgG0" title="YouTube video player" frameborder="0" allow="accelerometer;
...
def parse(s):
    new_link = "https://youtu.be/"
    if url := re.search(r".*src=\"(?:.+youtube).+embed/(.+)\".*", s):
        new_link += url.group(1)
    else:
        return None
    return new_link
