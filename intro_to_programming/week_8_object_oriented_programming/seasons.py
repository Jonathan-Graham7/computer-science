...
there are 365 x 24 x 60 = 525,600 minutes in that same year (because there are 24 hours in a day and 60 minutes in an hour).
...
as some of them could have 1 x 24 60 = 1,440 additional minutes.
...
def main():
    ...
    try:
        valid_date = isValid(input_date)
    except ValueError:
        sys.exit("Invalid date")
    print(f"{calculateMinutes(valid_date)} minutes")
...
def calculateMinutes(entry):
    diff_days = (date.today() - entry).days
    total_minutes = diff_days * 24 * 60
